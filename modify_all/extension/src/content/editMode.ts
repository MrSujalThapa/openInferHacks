import type { EditableGroup, PatchOperation, Rect } from "../../../shared/contracts";
...
import { applyOperations, buildOperationsFromManualEdit, getElementTransform } from "./patchEngine";
import { loadAndApplyCustomizations, saveCustomization, setupReapplyObserver } from "./persistence";
...
class EditModeController {
  ...
  private onPointerDown = (e: PointerEvent): void => {
    ...
    if (target.closest(".genie-selection-box") && this.selectedGroup) {
      if ((e.target as Element).closest(".genie-handle")) return;
  ...
  private async onLassoComplete(points: { x: number; y: number }[]): Promise<void> {
    ...
    const domSummary = elements.slice(0, 12).map((el, i) => summarizeElement(el, i)).filter(Boolean) as EditableGroup["domSummary"];
  ...
  private async saveCurrent(): Promise<void> {
    ...
    const ops =
      this.targetElement && this.operations.length
        ? buildOperationsFromManualEdit(this.selectedGroup.groupId, this.targetElement, this.operations)
        : this.operations;
